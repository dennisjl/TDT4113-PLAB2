...
# up letters and digits in this dictionary.
    ...
    #Henter signal og sender til prosessering. Skriver også ut 0-3
    def decoding_loop(self):
        while True:
            s = self.read_one_signal(self.serial_port)
            print(str(int(s)))
            for byte in s:
                self.process_signal(str(int(chr(byte))))

    #Tar inn signalet fra 0-3 og delegerer til riktig funksjon
    def process_signal(self,sig):
        if (sig == "0"):
            self.update_current_symbol("0")
        elif (sig == "1"):
            self.update_current_symbol("1")
        elif (sig == "2"):
